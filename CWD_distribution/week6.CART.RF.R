...
#===================================
# point-level data only for the model
#===================================
head(all_reduced,3)
...
pointtree <- randomForest(snag ~., 
                          data = train_point, 
                          ntree = 1000, 
                          importance = T)
(importance <- importance(pointtree))
varImpPlot(pointtree)
...
point4 <- point %>% filter(pool == 4) %>% dplyr::select(-pool)
  ...
  #make tree
  pointtree4 <- randomForest(snag ~., 
                            data = train_point_4, 
                            ntree = 1000, 
                            importance = T)
  varImpPlot(pointtree4)
...
point8 <- point %>% filter(pool == 8) %>% dplyr::select(-pool)
  ...
  #make tree
  pointtree8 <- randomForest(snag ~., 
                            data = train_point_8, 
                            ntree = 1000, 
                            importance = T)
  varImpPlot(pointtree8)
...
point13 <- point %>% filter(pool == 13) %>% dplyr::select(-pool)
  ...
  #make tree
  pointtree13 <- randomForest(snag ~., 
                            data = train_point_13, 
                            ntree = 1000, 
                            importance = T)
  varImpPlot(pointtree13)
