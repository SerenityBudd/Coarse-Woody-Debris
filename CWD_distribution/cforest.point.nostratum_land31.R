...
all_reduced <- all_reduced %>% dplyr::select(-c(stratum, barcode, uniq_id, Area, pct_prm_lotic, pct_prm_lentic, num_lotic_outl, num_lentic_outl, econ, pct_terr_shore_wetf, sinuosity, NEAR_TERR_CLASS_31_N.p, NEAR_FOREST_CLASS_31_N.p, year.p, stageht.p, stratum_name, snagyn, pct_area_le100, depth.p, current.p, substrt.p, trib.p, pct_aqveg, AQUA_CODE)) %>% na.omit()
all <- all_reduced
...
cv_error_all_nostratum <- mean(ers) # cross-validated mean error rate
...
pall <- plotnice(varimps_all, color = "olivedrab4", title = "Point RF var. importance (5fold), all pools")
ggsave(plot = pall, 
       "rf_avg_varimp_point_all_5foldcv_nostratum.png",
       width = 7, height = 4.5)
save(cv_error_all_nostratum, file = "data/cv_error_all_nostratum.Rda")
...
cv_error_pool4_nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p4, 
       "rf_avg_varimp_point_pool4_5foldcv_nostratum.png",
       width = 7, height = 4.5)
# save as rf_avg_varimp_point_pool4_5foldcv.png
save(cv_error_pool4_nostratum, file = "data/cv_error_pool4_nostratum.Rda")
...
cv_error_pool8_nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p8, 
       "rf_avg_varimp_point_pool8_5foldcv_nostratum.png",
       width = 7, height = 4.5)
# save as rf_avg_varimp_point_pool8_5foldcv.png
save(cv_error_pool8_nostratum, file = "data/cv_error_pool8_nostratum.Rda")
...
cv_error_pool13_nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p13, 
       "rf_avg_varimp_point_pool13_5foldcv_nostratum.png",
       width = 7, height = 4.5)
# save as rf_avg_varimp_point_pool13_5foldcv.png
save(cv_error_pool13_nostratum, file = "data/cv_error_pool13_nostratum.Rda")

cforest_errors_nostratum <- list(cv_error_all_nostratum, cv_error_pool4_nostratum, cv_error_pool8_nostratum, cv_error_pool13_nostratum)
save(cforest_errors_nostratum, file = "data/cforest_errors_nostratum.Rda")
