...
all_reduced <- all_reduced_15 %>% dplyr::select(-c(stratum, barcode, uniq_id, Area, pct_prm_lotic, pct_prm_lentic, num_lotic_outl, num_lentic_outl, econ, pct_terr_shore_wetf, sinuosity, NEAR_TERR_CLASS_15_N.p, NEAR_FOREST_CLASS_15_N.p, year.p, stageht.p, stratum_name, snagyn, pct_area_le100, depth.p, current.p, substrt.p, trib.p, pct_aqveg, AQUA_CODE)) %>% na.omit()
all <- all_reduced
...
predictorvars_all <- c("pool", "perimeter", "max_depth", "avg_depth", "total_volume", "shoreline_density_index", "pct_terrestrial_shore", "pct_perimeter_wetforest", "dist_to_land", "nearest_land_class", "dist_to_forest", "nearest_forest_class", "wingdam", "revetment")
...
cv_error_all_landclass15nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p, "rf_avg_varimp_point_all_5foldcv_landclass15nostratum.png",
       width = 7, height = 4.5)
save(cv_error_all_landclass15nostratum, file = "data/cv_error_all_landclass15nostratum.Rda")
...
predictorvars_pools <- c("perimeter", "max_depth", "avg_depth", "total_volume", "shoreline_density_index", "pct_terrestrial_shore", "pct_perimeter_wetforest", "dist_to_land", "nearest_land_class", "dist_to_forest", "nearest_forest_class", "wingdam", "revetment") #define vars for the pools
...
cv_error_pool4_landclass15nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p4, 
       "rf_avg_varimp_point_pool4_5foldcv_landclass15nostratum.png",
       width = 7, height = 4.5)
save(cv_error_pool4_landclass15nostratum, file = "data/cv_error_pool4_landclass15nostratum")
...
cv_error_pool8_landclass15nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p8, 
       "rf_avg_varimp_point_pool8_5foldcv_landclass15nostratum.png",
       width = 7, height = 4.5)
save(cv_error_pool8_landclass15nostratum, file = "data/cv_error_pool8_landclass15nostratum.Rda")
...
cv_error_pool13_landclass15nostratum <- mean(ers) # cross-validated mean error rate
...
ggsave(plot = p13, 
       "rf_avg_varimp_point_pool13_5foldcv_landclass15nostratum.png",
       width = 7, height = 4.5)
save(cv_error_pool13_landclass15nostratum, file = "data/cv_error_pool13_landclass15nostratum.Rda")

cforest_errors_landclass15nostratum <- list(cv_error_all_landclass15nostratum, cv_error_pool4_landclass15nostratum, cv_error_pool8_landclass15nostratum, cv_error_pool13_landclass15nostratum)
save(cforest_errors_landclass15nostratum, file = "data/cforest_errors_landclass15nostratum.Rda")
